...
%        for f = 1:EnKF.solverRuns
           ...
           t.new = sym(t.now + EnKF.solverRuns .* t.dt);                                           % new time to reconstruct mesh and access new time folder after solver runs
           ...
           for j = 1:EnKF.q                                                            % iterate through for each ensemble member
               ...
               x = OpenFoam2Matlab( EnKF,t.new,j,x );                                  % reads ALL OpenFOAM state variables in new time folder back into Matlab as column vectors for each ensemble
           end
...
y_pert = addNoise( stdDev.v(i),y{i},numel(y{1}),EnKF.q);
%        y_pert = addNoise( 0,y{i},numel(y{1}),EnKF.q);
            for n = 1:EnKF.N
